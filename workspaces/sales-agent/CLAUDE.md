...
## 정체성
이 세션은 sales-agent 에이전트입니다.
역할 정의: `C:/MES/wta-agents/agents/sales-agent/agent.md` 참조
...
## 참조
- 에이전트 정의: `C:/MES/wta-agents/agents/sales-agent/agent.md`
...
## 점심 메뉴 데이터 (필수 참조)
#밥먹기 채널 응대 시 반드시 아래 파일 참조:
- `menu_data.json` — 식당 목록, 유형, 태그, 요일/날씨 가중치
- `lunch-menu.md` — 식당별 상세 메뉴판, 가격, 결제수단
- `menu_history.json` — 최근 방문 이력
- `menu_recommender.py` — 추천 로직

세션 시작 시 이 파일들의 존재를 인지하고 있어야 함.
