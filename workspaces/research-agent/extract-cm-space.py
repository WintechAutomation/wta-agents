"""
CM 스페이스 전체 추출 스크립트
- 767개 페이지 본문(HTML 변환) + 이미지 다운로드
- 저장: C:/MES/wta-agents/reports/MAX/confluence-CM/{pageId}-{title}/index.html + images/
"""
...
PROGRESS_FILE = 'C:/MES/wta-agents/workspaces/research-agent/cm-extract-progress.json'
TIMEOUT = 30
MAX_RETRIES = 3
...
def adf_to_html(node, images_dir_name='images'):
    ...
    if ntype == 'doc':
        ...
        items = ''.join(f'<li>{"".join(adf_to_html(c, images_dir_name) for c in item.get("content", []))}</li>'
        ...
        for c in content:
            if c.get('type') == 'media':
                ...
                if media_id:
                    # 파일명은 media_id로 매핑 (다운로드 후 업데이트)
                    parts.append(f'<figure><img src="{images_dir_name}/{media_id}" alt="{alt}" style="max-width:100%;border:1px solid #ddd;border-radius:4px;margin:8px 0;"><figcaption style="font-size:12px;color:#666;">{alt}</figcaption></figure>')
        return '\n'.join(parts)
        ...
        inner = ''.join(adf_to_html(c, images_dir_name) for c in content)
        style = 'border:1px solid #ddd;padding:6px 10px;' + ('background:#f4f5f7;font-weight:bold;' if ntype == 'tableHeader' else '')
...
def process_page(page_id, page_title):
    ...
    try:
        ...
        print(f'  ERR 본문조회 [{page_id}] {page_title}: {e}')
        return False

    if r.status_code != 200:
        print(f'  ERR 본문조회 [{page_id}] {page_title}: HTTP {r.status_code}')
        return False
    ...
    # 2. 첨부파일(이미지) 목록 조회
    att_url = f'{BASE}/wiki/rest/api/content/{page_id}/child/attachment?limit=100&mediaType=image'
    try:
    ...
    # media ID → 파일명 매핑 (ADF에서 id 속성으로 참조)
    media_id_map = {}  # media_id → filename
    ...
    for att in attachments:
        if not att.get('metadata', {}).get('mediaType', '').startswith('image/'):
            continue
        att_id = att['id']  # 'att12345' 형태
        filename = att['title']
        # ADF에서 media node의 id는 UUID 형태일 수 있음
        # attachment extensions의 fileId 확인
        ...
        try:
            ...
            if img_r.status_code == 200 and len(img_r.content) > 0:
                ...
                print(f'    WARN 이미지 실패 [{att_id}] {filename}: {img_r.status_code}')
        except Exception as e:
            print(f'    WARN 이미지 오류 [{att_id}] {filename}: {e}')
        time.sleep(0.1)

    # 4. ADF → HTML 변환 (media id를 실제 파일명으로 치환)
    body_html = adf_to_html(adf, 'images')

    # media_id 참조를 실제 파일명으로 치환
    for media_id, fname in media_id_map.items():
        body_html = body_html.replace(f'images/{media_id}"', f'images/{fname}"')

    # 미해결 media_id (UUID 패턴) 제거
...
# 메인 실행
with open('C:/MES/wta-agents/workspaces/research-agent/cm-pages-all.json', encoding='utf-8') as f:
...
print(f'총 {total}개 페이지 추출 시작 (이미 완료: {len(done_set)}개)')
...
for i, page in enumerate(all_pages):
    ...
    if (i + 1) % 10 == 0:
        save_progress()
        print(f'  → 진행: {success}완료/{failed}실패/{total}전체')
