"""
경상연구개발 5개 주제 Confluence HTML 추출
기존 CM/UserManual/GCSSETUP/ISSUE 방식과 완전 동일:
  {주제}/{pageId}-{title}/
    index.html
    images/
"""
...
PROGRESS_FILE = Path('C:/MES/wta-agents/workspaces/research-agent/research-html-progress.json')
...
def process_page(page_id, page_title, topic_dir):
    ...
    space_key = page_data.get('space', {}).get('key', '') if isinstance(page_data.get('space'), dict) else ''
...
for topic, pages in all_pages.items():
    ...
    already = sum(1 for p in pages if p['id'] in done_set)
    print(f'\n[{topic}] {total}개 (이미 완료: {already}개)')
    ...
    for i, page in enumerate(pages):
        ...
        if (i + 1) % 10 == 0:
            ...
            done_count = total_success + t_success
            rate = done_count / elapsed if elapsed > 0 else 1
            remaining = (149 - done_count) / rate if rate > 0 else 0
            print(f'  → 진행: {t_success}완료/{t_failed}실패 | 예상잔여: {remaining/60:.1f}분')

        time.sleep(0.3)

    save_progress()
    total_pages_in_dir = len([d for d in (OUTPUT_BASE / topic).iterdir() if d.is_dir()]) if (OUTPUT_BASE / topic).exists() else 0
    print(f'[{topic}] 완료: {t_success}성공 {t_failed}실패 (폴더:{total_pages_in_dir}개)')
