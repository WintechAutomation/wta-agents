"""
CVD L&UL Machine EN - 매뉴얼 텍스트 기반 이미지 카탈로그
83 Figure captions from parsed text, matched to images in DOCX extraction order
...
PARSED = Path(r'C:\MES\wta-agents\data\wta_parsed\HAM-CVD_L_UL_Machine_User_Manual_en_v1.0.md')
...
def extract_figures(md_path: Path) -> list[tuple[int, str, str]]:
    ...
    for i, line in enumerate(lines):
        ...
        if m:
            ...
            figures.append((i + 1, caption, current_section))
    return figures
...
def main():
    ...
    # Split into User Manual (line < 1100) and Maintenance Manual
    user_figs = [(ln, cap, sec) for ln, cap, sec in figures if ln < 1100]
    maint_figs = [(ln, cap, sec) for ln, cap, sec in figures if ln >= 1100]
    ...
    seen_hashes = {}
    figure_imgs = []  # (path, w, h, is_duplicate)
    small_imgs = []
    icon_imgs = []
    ...
    # 4. Split unique figures into user manual / maintenance manual halves
    # The manual has ~100 images for user manual, ~100 for maintenance
    # Images roughly img_001-img_100 = user manual, img_100+ = maintenance
    # Use img number to split
    def get_img_num(path):
        m = re.search(r'(\d+)', path.stem)
        return int(m.group(1)) if m else 0

    # Find the split point - maintenance manual starts around figure 42
    # which maps to approximately img_042+
    # But we need to match 41 user figs + 42 maint figs

    # Strategy: sequential match unique figures to captions
    # First 41 unique figures -> user manual captions
    # Remaining unique figures -> maintenance manual captions
    ...
    # Get unique figures for user manual (first batch)
  ...
  <img src="{thumb}" alt="Figure - {cap}"/>
  <div class="cap">Figure - {cap}</div>
...
<head>
<meta charset="UTF-8">
<title>CVD L&UL Machine (EN) — Image Catalog</title>
<style>
...
<body>
<h1>CVD L&UL Machine (EN) — Manual Image Catalog</h1>
<div class="info">
